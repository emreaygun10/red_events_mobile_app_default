import 'package:red_events_mobile_app_defult/feature/profile/view/request_views/state/profile_request_change_shift_state.dart';
import 'package:red_events_mobile_app_defult/product/state/base/base_cubit.dart';

/// Profile Request Report state management class
class ProfileRequestChangeShiftBloc
    extends BaseCubit<ProfileRequestChangeShiftState> {
  ///
  ProfileRequestChangeShiftBloc({
    required List<String> managerList,
    required List<String> dayList,
  }) : super(
          ProfileRequestChangeShiftState(
            isLoading: false,
            managerList: managerList,
            isButtonEnable: false,
            dayList: dayList,
          ),
        );

  /// change value group index
  void changeGroupIndex(int value) {
    emit(
      state.copyWith(
        groupIndex: value,
      ),
    );
  }

  ///button clickable changer
  void changeButtonEnable({required bool isEnable}) {
    emit(
      state.copyWith(isButtonEnable: isEnable),
    );
  }

  /// change value explanation
  void valueChangedExplanation(String? explanation) {
    //print(explanation);
    emit(
      state.copyWith(
        explanation: explanation,
      ),
    );
  }

  /// Fill Manager List
  bool addManagerList(String manager) {
    // ignore: prefer_final_locals
    var tempList = <String>[];
    if (state.managerCheckBoxList.length < 2) {
      for (final name in state.managerCheckBoxList) {
        tempList.add(name);
      }
      tempList.add(manager);
      emit(state.copyWith(managerCheckBoxList: tempList));
      return true;
    } else {
      return false;
    }
  }

  /// Fill Day List
  bool addDayList(String day) {
    // ignore: prefer_final_locals
    var tempList = <String>[];
    for (final name in state.dayCheckBoxList) {
      if (!tempList.contains(name)) {
        tempList.add(name);
      }
    }
    tempList.add(day);
    emit(state.copyWith(dayCheckBoxList: tempList));
    return true;
  }

  /// Remove manager list
  void removeManagerList(String manager) {
    // ignore: prefer_final_locals
    var tempList = <String>[];
    for (final name in state.managerCheckBoxList) {
      tempList.add(name);
    }
    tempList.remove(manager);
    emit(state.copyWith(managerCheckBoxList: tempList));
  }

  /// Remove day list
  void removeDayList(String day) {
    // ignore: prefer_final_locals
    var tempList = <String>[];
    for (final name in state.dayCheckBoxList) {
      tempList.add(name);
    }
    tempList.remove(day);
    emit(state.copyWith(dayCheckBoxList: tempList));
  }
}
